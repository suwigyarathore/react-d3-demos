...
export const StyledBar = () =>  {
    ...
    const yScale = barData && scaleBand().domain(barData.map(d => d.Country)).range([0, innerHeight]).paddingInner(0.5);
    ...
    return (
       (dataAvailable) ? <svg width={width} height={height}>
           <g  transform={`translate(${margin.left}, ${margin.top})`}>
              <AxisBottom xScale={xScale} innerHeight={innerHeight} tickFormat={xTickFormat}/>
              <AxisLeft yScale={yScale}  />
              <text className="axis-label" x={innerWidth / 2} y={innerHeight + xLabelOffest} textAnchor="middle" fontSize="20" fill="black">Population</text>
              <Marks 
                data={barData} 
                yScale={yScale} 
                xScale={xScale} 
